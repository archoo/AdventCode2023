...
#rawdata = [r.strip().split() for r in open('day07_test.txt').readlines()]
...
def fixhand(h):
  nextbest = Counter(h.replace('J','')).most_common(1)
  nextbest.append(('J',0))
  print(nextbest)
  h = h.replace('J',nextbest[0][0])
  return h
...
for h in hands:
  ...
  crank = [cards.index(c) for c in hand]
